<?php
	...
	function new_application($id_user_from, $id_user_to) {
		global $db;
		
		// Verify if exist
		if(is_user_already_applied($id_user_from, $id_user_to)) {
			return false;
		}

		$q = $db->prepare ( "INSERT INTO mm_users_applications (id_user_from, id_user_to) VALUES (?, ?)" );
		$q->bind_param ( 'ii', $id_user_from, $id_user_to);
	...
	function is_user_already_applied($id_user_from, $id_user_to) {
		global $db;
		
		$q = $db->prepare ( "SELECT * FROM mm_users_applications WHERE id_user_from = ? AND id_user_to = ? LIMIT 1" );
		$q->bind_param ( 'ii', $id_user_from, $id_user_to );
		$q->execute();
		$result = $q->get_result();
	
		while ( $o = $result->fetch_array(MYSQLI_ASSOC) ) {	
			if ( $o['id_user_from'] == $id_user_from && $o['id_user_to'] == $id_user_to ) {
				return true;
			}		
		}
		return false;
	 ...
	 */
	function get_user_application($type, $query = '', $extra = '') {
		global $db;
		...
		if(is_array($query)) {
			...
			$q = $db->prepare ( "SELECT * FROM mm_users_applications $build_query $extra" );
			...
			$q = $db->prepare ( "SELECT * FROM mm_users_applications WHERE id_application = ? LIMIT 1" );
