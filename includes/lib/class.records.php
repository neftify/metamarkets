<?php
	...
	function get_record_by_short_description($short_description) {
		return get_ngc_record('one', array(
			0 => array("type" => "CHR", "condition" => "AND", "loose" => false, "table" => "short_description", "command" => "=", "value" => $short_description)
		));
	}
	...
	function new_record($description, $value) {
		...
		$q = $db->prepare ( "INSERT INTO mm_record (short_description, `value`) VALUES (?, ?)" );
	...
	function get_ngc_record ($type, $query = '', $extra = '') {
		...
		if(is_array($query)) {
			...
			$q = $db->prepare ( "SELECT * FROM mm_record $build_query $extra" );
			...
			$q = $db->prepare ( "SELECT * FROM mm_record WHERE id_record = ? LIMIT 1" );
