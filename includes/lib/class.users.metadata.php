<?php
	...
	function update_metadata($id_user, $metadata, $value) {
		global $db;

		$q = $db->prepare ( "UPDATE mm_users_metadata SET `value` = ? WHERE id_user = ? AND `name` = ?" );
		$q->bind_param ( 'sis', $value, $id_user, $metadata );		
	...
	function new_metadata($id_user, $metadata, $value) {
		global $db;
		
		// Verify if exist
		if(get_metadata_by_user_id($id_user, $metadata)) {
			return false;
		}

		$q = $db->prepare ( "INSERT INTO mm_users_metadata (id_user, `name`, `value`) VALUES (?, ?, ?)" );
		$q->bind_param ( 'iss', $id_user, $metadata, $value);
	 ...
	 */
	function get_user_metadata($type, $query = '', $extra = '') {
		global $db;
		...
		if(is_array($query)) {
			...
			$q = $db->prepare ( "SELECT * FROM mm_users_metadata $build_query $extra" );
			...
			$q = $db->prepare ( "SELECT * FROM mm_users_metadata WHERE id_metadata = ? LIMIT 1" );
